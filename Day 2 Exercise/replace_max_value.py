...
# Find maximum value in a linked list and replace with given value
def replace_maximum(number_list,new_value):
    temp = number_list.get_head() # Temporary Value
    max = 0
    while (temp is not None):
        if (max < temp.get_data()):
            max = temp.get_data()
        temp = temp.get_next()
    number_list.insert(new_value, max)
    number_list.delete(max)
    return number_list
